...
function getGitHubRepoInfo(packageObj) {
  ...
  const repoSearchString = `https://api.github.com/search/repositories?q=${packageObj.repoName}+fork:false+language:js+repo:${fullName}+access_token=${access_token}`

  return fetch(repoSearchString, {
    headers: {
      // Has to add 'User-Agent' header. Otherwise GitHub won't accept the request
      'User-Agent': 'Package Info Atom Plugin',
    },
  }).then(res => {
    // const resBody = res.json().body
    console.log(res)
    const resultItem = resBody.items[0]
    console.log(resultItem)
    return pick(resultItem, 'created_at', 'description', 'forks_count', 'html_url', 'stargazers_count')
...
async function submitQuery(query) {
  ...
  const githubResults = await Promise.all(
    npmResults.map(({url}) => extractGitHubRepoName(url))
    .map(pkgObj => getGitHubRepoInfo(pkgObj))
  )
